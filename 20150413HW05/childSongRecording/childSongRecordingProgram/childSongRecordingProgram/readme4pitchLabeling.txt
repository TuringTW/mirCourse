...
0. 修改 goPitchLabel.m 以將所需要的 toolbox 加到搜尋路徑之中。
1. 修改 goPitchLabel.m，將 waveDir 指到你之前的錄音檔案所在的目錄。 
2. 執行 goPitchLabel，即可產生一個視窗，對第一個 wave 檔案進行標示 pitch 的工作。
3. 請將此視窗放到最大，以便觀察、點選。
...
11. 你可以點選 Play computed pitch 以播放電腦自動找到的音高。
11. 你可以點選 Play wave & computed pitch 以播放歌聲以及電腦自動找到的音高。
12. 你可以點選 Play labeled pitch 以播放修正後的音高曲線，不應該有暴音。
12. 你可以點選 Play wave & labeled pitch 以播放歌聲以及人工修正後的音高曲線。
13. 你可以點宣 Save pitch 來將人工摽示的音高向量儲存到附檔名為 pv 的檔案。
14. 每次標示完一首歌曲，按下 Enter，即自動儲存最後人工標示的 pitch 檔案，並可跳到下一首。
